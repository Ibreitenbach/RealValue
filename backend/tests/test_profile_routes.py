...
def test_get_my_profile_authenticated_existing_profile(client):
    ...
    print("Response data for test_get_my_profile_authenticated_existing_profile:", response.data)
...
def test_update_my_profile_partial_data(client):
    ...
    response_name = client.put("/api/profile/me", headers=headers, json=update_data_name)
    ...
    assert data_bio["display_name"] == "Just Name Change" # Name should persist from previous update
...
def test_update_my_profile_empty_json(client):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.put("/api/profile/me", headers=headers, data="{}", content_type="application/json") # Empty JSON
    assert response.status_code == 200 # Should succeed, making no changes or using defaults
...
def test_update_my_profile_no_json_body(client):
    ...
    data = json.loads(response.data)
    assert "Invalid or missing JSON in request" in data["msg"] # Updated expected message
...
def test_login_creates_profile_if_not_exists(client):
    ...
    assert UserProfile.query.filter_by(user_id=user.id).first() is None # Ensure no profile
