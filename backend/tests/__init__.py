...
def create_app():
    ...
    if not app.debug and not app.testing:
        ...
        file_handler = RotatingFileHandler(
            "logs/realvalue.log", maxBytes=10240, backupCount=10
        )
    ...
    from .routes.endorsement_routes import (
        endorsement_bp,
    )  # Assuming endorsement routes are available
    ...
    from .routes.journal_routes import (
        journal_bp,
    )  # Assuming journal routes are available

    app.register_blueprint(journal_bp)

    from .routes.mindset_routes import (
        mindset_bp,
    )  # Assuming mindset routes are available

    app.register_blueprint(mindset_bp)

    from .routes.mindful_moment_routes import (
        mindful_moments_bp,
    )  # Assuming mindful moments routes are available

    app.register_blueprint(mindful_moments_bp)

    from .routes.mind_content_routes import (
        mind_content_bp,
    )  # Assuming mind content routes are available
